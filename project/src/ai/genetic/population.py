...
def breed3(mother: Individual, father: Individual, slice_idx1, slice_idx2) -> Individual:
    ...
    moves = mother.get_genes()[:slice_idx1] + father.get_genes()[slice_idx1:slice_idx2] + mother.get_genes()[slice_idx2:]
    child = Individual()
    child.set_genes(moves)
    return child
...
class Population:
    ...
    def select_by_roulette(self) -> None:
        ...
        for i in range(1, len(self.population)):
            segments[i] = (segments[i - 1] + self.population[i].get_fitness() + 1)
        new_population = []
