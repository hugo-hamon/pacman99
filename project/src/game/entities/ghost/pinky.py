...
class Pinky(GeneralGhost):
    ...
    def get_chase_direction(self) -> Direction:
        ...
        for direction in self.get_possible_direction():
            ...
            dist_to_dir = self.distance_euclidienne(dir_ghost_pos, \
                (lambda x, y: (x[0] + self.PACMAN_OFFSET * y[0], x[1] + self.PACMAN_OFFSET * y[1])) (pac_pos, self.pac.get_direction().to_vector()))
