...
class RandomMazeFactory():
    ...
    def __create_graph(self) -> None:
        ...
        # Each element of the graph is a list of tuples (node, direction)
        g = {}
        # Number of nodes
        n = (self.height // self.intersection_step) * (self.width // self.intersection_step)
        ...
        for node in g.keys():
            ...
            candidates = [(node + 1, Direction.EAST), (node - 1, Direction.WEST), (node + self.width // self.intersection_step, Direction.SOUTH), (node - self.width // self.intersection_step, Direction.NORTH)]
            # Add every candiate to the graph
            # Now every intersection is connected to every potential neighbor
            for i in range(2):
                if candidates[i][0] in g.keys() and (((candidates[i][0] - 1) * self.intersection_step) // self.width) + 1 == (((node - 1) * self.intersection_step) // self.width) + 1:
                    g[node].append(candidates[i])
                if candidates[i + 2][0] in g.keys():
        ...
        for node in g.keys():
            for i in range(4):
                ...
                if random.random() > self.density:
                    ...
                    if len(g[vertex_to_remove[0]]) - 1 >= 2:
                        g[node].remove(vertex_to_remove)
                        g[vertex_to_remove[0]].remove((node, vertex_to_remove[1].opposite()))
        return g
    ...
    def __add_ghost_box(self, maze) -> None:
        ...
        if self.is_symetric:
            ...
            for j in range(1,len(maze) - 2):
                ...
                maze[j, x + len(self.GHOST_BOX[0]) - self.intersection_step] = Components.DOT
        # Add the ghost box
        ...
        for i in range(1,len(self.GHOST_BOX) - 1):
            # Left side
            if maze[y + i, x - 1] == Components.DOT:
                if maze[y + i, x - 2] != Components.DOT:
                    maze[y + i, x - 1] = Components.WALL
            # Right side
            if maze[y + i, x + len(self.GHOST_BOX[0])] == Components.DOT:
                if maze[y + i, x + len(self.GHOST_BOX[0]) + 1] != Components.DOT:
                    maze[y + i, x + len(self.GHOST_BOX[0])] = Components.WALL
        for j in range(1,len(self.GHOST_BOX[0]) - 1):
            # Top side
            if maze[y - 1, x + j] == Components.DOT:
                if maze[y - 2, x + j] != Components.DOT:
                    maze[y - 1, x + j] = Components.WALL
            # Bottom side
            if maze[y + len(self.GHOST_BOX), x + j] == Components.DOT:
                if maze[y + len(self.GHOST_BOX) + 1, x + j] != Components.DOT:
                    maze[y + len(self.GHOST_BOX), x + j] = Components.WALL
        return maze


    def __graph_to_maze(self, g) -> None:
        ...
        for k, v in g.items():
            ...
            y = (((k - 1) * self.intersection_step) // self.width) * self.intersection_step + 1
            ...
            for i, direction in v:
                ...
                yi = (((i - 1) * self.intersection_step) // self.width) * self.intersection_step + 1
                xv = x
                yv = y
                while (xi, yi) != (xv, yv):
